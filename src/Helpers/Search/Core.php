...
class Core
{
    ...
    {
        return IndexProducts::query()->select('index_products.id_product')->join('product_index', 'index_products.id_product', '=', 'product_index.id_product')
            ->where('index_products.id_index_catalog', $index)->where('index_products.value', 'like', '%' . $query . '%')->where('index_products.status', 1)
            ->where('product_index.status', 1)->orderBy('index_products.index_priority', 'desc')->pluck('index_products.id_product')
